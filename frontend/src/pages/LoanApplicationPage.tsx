...
export default function LoanApplicationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!memberId) { setError(t('loans.pleaseSelectMember')); return; }
    setSubmitting(true);
    setError('');
    try {
      ...
      setSuccess(t('loans.applicationSuccess'));
      setTimeout(() => navigate('/loans'), 1500);
    } catch (err: any) {
      setError(err.response?.data?.message || err.response?.data || t('loans.applicationFailed'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-8">
        <h2 className="text-2xl font-black text-text-dark dark:text-slate-100">{t('loans.apply')}</h2>
        <p className="text-sm text-gray-500 mt-1">{t('loans.submitApplication')}</p>
      </div>
      ...
      {principalAmount && parseFloat(principalAmount) > 0 && interestRate && durationMonths && (
        <div className="mb-5 rounded-xl border border-blue-100 bg-primary-50 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold text-blue-900">{t('loans.paymentEstimate')}</span>
          </div>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-xs text-blue-600">{t('loans.monthlyPayment')}</p>
              <p className="text-lg font-bold text-blue-900">${Number(calcMonthlyPayment()).toLocaleString()}</p>
            </div>
            <div>
              <p className="text-xs text-blue-600">{t('loans.totalInterest')}</p>
              <p className="text-lg font-bold text-blue-900">${Number(calcTotalInterest()).toLocaleString()}</p>
            </div>
            <div>
              <p className="text-xs text-blue-600">{t('loans.totalRepayment')}</p>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-blue-50 dark:border-slate-700 p-6 space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {isCustomer ? (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('common.member')}</label>
              <input type="text" readOnly value={t('loans.youSelfService')}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('loans.loanType')} *</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('loans.principalAmount')} *</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('loans.interestRate')} *</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('loans.durationMonths')} *</label>
          <input type="number" min="1" max="360" required value={durationMonths} onChange={e => setDurationMonths(e.target.value)}
            className="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/15 focus:border-primary transition-all text-sm" />
          <p className="mt-1 text-xs text-gray-400">{t('loans.durationHint')}</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('loans.collateralType')}</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('loans.collateralDescription')}</label>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="button" onClick={() => navigate('/loans')}
            className="flex-1 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-xl font-medium hover:bg-gray-200 transition-colors">{t('common.cancel')}</button>
        </div>
      </form>
    </div>
  );
