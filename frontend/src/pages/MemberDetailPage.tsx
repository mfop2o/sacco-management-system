...
export default function MemberDetailPage() {
  ...
  const handleDeposit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTxMsg({ text: t('members.depositSuccessful'), type: 'success' });
      ...
      setTxMsg({ text: t('members.depositFailed'), type: 'error' });
    }
  };
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTxMsg({ text: t('members.withdrawalSuccessful'), type: 'success' });
      ...
      setTxMsg({ text: t('members.withdrawalFailed'), type: 'error' });
    }
  };
  ...
  if (!member) return <div className="text-center py-20 text-gray-500">{t('members.memberNotFound')}</div>;
  ...
  return (
    <div>
      <div className="mb-6">
        <Link to="/members" className="text-sm text-gray-500 hover:text-blue-800 transition-colors">&larr; {t('members.backToMembers')}</Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-blue-50 dark:border-slate-700 p-6">
          <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">{t('members.memberInfo')}</h3>
          <dl className="space-y-3">
            {[
              [t('members.membershipNumber'), member.membershipNumber],
              [t('members.phone'), member.phone],
              [t('members.email'), member.email || '-'],
              [t('members.joined'), member.joinedDate],
            ].map(([label, value]) => (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-blue-50 dark:border-slate-700 p-6">
          <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">{t('members.savingsAccounts')}</h3>
          {accounts.length === 0 ? (
            <p className="text-sm text-gray-500 text-center py-4">{t('members.noAccounts')}</p>
          ) : (
                ...
                <div key={acc.id} className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                  ...
                  <div className="flex gap-2 mt-3">
                    <button onClick={() => { setSelectedAccountId(acc.id); setShowDeposit(true); setTxMsg({ text: '', type: '' }); }}
                      className="flex-1 px-3 py-1.5 text-xs font-semibold bg-primary-100 text-primary rounded-xl hover:bg-primary-200">{t('members.deposit')}</button>
                    <button onClick={() => { setSelectedAccountId(acc.id); setShowWithdraw(true); setTxMsg({ text: '', type: '' }); }}
                      className="flex-1 px-3 py-1.5 text-xs font-semibold bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200">{t('members.withdraw')}</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-blue-50 dark:border-slate-700 p-6">
          <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">{t('members.summary')}</h3>
          <div className="space-y-4">
            <div className="bg-primary-50 rounded-xl p-4">
              <p className="text-xs text-blue-900 font-medium">{t('members.totalSavings')}</p>
              <p className="text-2xl font-black text-text-dark dark:text-slate-100 mt-1">${totalSavings.toLocaleString()}</p>
            </div>
            <div className="bg-gray-100 rounded-xl p-4">
              <p className="text-xs text-gray-600 font-medium">{t('members.activeLoans')}</p>
            ...
            <div className="bg-gray-100 rounded-xl p-4">
              <p className="text-xs text-gray-600 font-medium">{t('members.outstanding')}</p>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-blue-50 dark:border-slate-700 p-6 mb-6">
        <div className="flex items-center gap-4 mb-4 border-b border-gray-200 pb-4">
          <h3 className="text-lg font-bold text-text-dark">{t('members.loansAndTransactions')}</h3>
          <div className="flex gap-1 bg-gray-100 rounded-lg p-0.5">
            <button onClick={() => setActiveTab('loans')} className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-all ${activeTab === 'loans' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{t('members.loans')}</button>
            <button onClick={() => setActiveTab('transactions')} className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-all ${activeTab === 'transactions' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{t('members.transactions')}</button>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b border-gray-200">
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('loans.loanNumber')}</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('common.type')}</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('loans.amount')}</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('common.balance')}</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('common.status')}</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('common.date')}</th>
              </tr></thead>
              <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                {loans.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-500">{t('members.noLoans')}</td></tr>}
              </tbody>
            </table>
          </div>
        )}
        ...
        {activeTab === 'transactions' && (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b border-gray-200">
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('transactions.txNumber')}</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('common.type')}</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('common.amount')}</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('transactions.description')}</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('common.date')}</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-text-soft uppercase">{t('common.status')}</th>
              </tr></thead>
              <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                {transactions.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">{t('members.noTransactions')}</td></tr>}
              </tbody>
            </table>
          </div>
        )}
      ...
      <Modal show={showDeposit} onClose={() => setShowDeposit(false)} title={t('members.depositToAccount')} type="deposit">
        <form onSubmit={handleDeposit}>
          ...
          <div className="flex gap-3">
            <button type="submit" className="flex-1 px-4 py-2.5 bg-primary text-white rounded-2xl font-semibold hover:bg-primary-dark transition-colors">{t('members.deposit')}</button>
            <button type="button" onClick={() => setShowDeposit(false)} className="flex-1 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-2xl font-semibold hover:bg-gray-200 transition-colors">{t('common.cancel')}</button>
          </div>
        </form>
      </Modal>

      <Modal show={showWithdraw} onClose={() => setShowWithdraw(false)} title={t('members.withdrawFromAccount')} type="withdraw">
        <form onSubmit={handleWithdraw}>
          ...
          <div className="flex gap-3">
            <button type="submit" className="flex-1 px-4 py-2.5 bg-primary text-white rounded-2xl font-semibold hover:bg-primary-dark transition-colors">{t('members.withdraw')}</button>
            <button type="button" onClick={() => setShowWithdraw(false)} className="flex-1 px-4 py-2.5 bg-gray-100 text-gray-700 rounded-2xl font-semibold hover:bg-gray-200 transition-colors">{t('common.cancel')}</button>
          </div>
        </form>
      </Modal>
    </div>
